<div align="center">

![Header](https://capsule-render.vercel.app/api?type=waving&color=0:0f0c29,50:302b63,100:24243e&height=200&section=header&text=Md%20Khan%20Bahadur%20Sadi&fontSize=40&fontColor=ffffff&fontAlignY=38&desc=Software%20Developer%20%E2%80%A2%20AI%20Enthusiast%20%E2%80%A2%20CSE%20Student&descSize=16&descAlignY=60&animation=fadeIn)

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&duration=3000&pause=1000&color=6C63FF&center=true&vCenter=true&multiline=false&width=600&lines=Building+scalable+software+solutions+%F0%9F%9A%80;Exploring+Machine+Learning+%26+AI+%F0%9F%A4%96;Future+Global+Tech+Brand+%E2%80%94+ITHLAYNIT+%F0%9F%8C%8D;Driven+by+faith%2C+purpose+%26+discipline+%E2%9C%A8)](https://git.io/typing-svg)

<br />

[![Website](https://img.shields.io/badge/🌐_Personal_Website-24243e?style=for-the-badge&logoColor=white)](http://www.mdkhanbahadursadi.ami.bd/)
[![Email](https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:khanbahadursadi@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/MdKhanBahadurSadi)

![Profile Views](https://komarev.com/ghpvc/?username=MdKhanBahadurSadi&color=6C63FF&style=for-the-badge&label=PROFILE+VIEWS)

</div>

---

## 🧑‍💻 About Me

```python
class KhanBahadurSadi:
    def __init__(self):
        self.name = "Md Khan Bahadur Sadi"
        self.role = "Software Developer & CSE Student"
        self.university = "North Western University, Khulna"
        self.location = "Khulna, Bangladesh 🇧🇩"
        self.focus = ["Machine Learning", "AI Systems", "Scalable Software"]
        self.brand = "ITHLAYNIT 🌍"
        self.faith = "Al-Islam — Qur'an & Sunnah 🕌"

    def current_status(self):
        return "Building impactful software · Exploring AI · Pursuing excellence"
```

> _"Indeed, my prayer, my sacrifice, my living and my dying are for Allah, Lord of the worlds."_  
> — **Al-Qur'an 6:162**

---

## 🎓 Academic Background

| Field | Details |
|---|---|
| 🎓 **Degree** | BSc in Computer Science & Engineering *(Running)* |
| 🏫 **University** | North Western University, Khulna |
| 📘 **HSC** | Science — 2022 |
| 📘 **SSC** | Science — 2020 |

---

## ⚙️ Technical Skills

<table>
  <tr>
    <td width="50%" valign="top">

### 💻 Languages

<p>
  <img src="https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white" />
  <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white" />
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/Dart-0175C2?style=for-the-badge&logo=dart&logoColor=white" />
</p>

    </td>
    <td width="50%" valign="top">

### 🌐 Web Development

<p>
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white" />
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" />
  <img src="https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white" />
</p>

    </td>
  </tr>

  <tr>
    <td width="50%" valign="top">

### 📱 Mobile & Backend

<p>
  <img src="https://img.shields.io/badge/Flutter-02569B?style=for-the-badge&logo=flutter&logoColor=white" />
  <img src="https://img.shields.io/badge/Express.js-000000?style=for-the-badge&logo=express&logoColor=white" />
  <img src="https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white" />
</p>

    </td>
    <td width="50%" valign="top">

### 🛠️ Tools & Platforms

<p>
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
  <img src="https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white" />
  <img src="https://img.shields.io/badge/IntelliJ_IDEA-000000?style=for-the-badge&logo=intellij-idea&logoColor=white" />
  <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black" />
</p>

    </td>
  </tr>
</table>

---

## 📊 Skill Proficiency

<table>
  <tr>
    <td><strong>HTML5</strong></td>
    <td width="70%"><progress value="90" max="100"></progress></td>
    <td>90%</td>
  </tr>
  <tr>
    <td><strong>Tailwind CSS</strong></td>
    <td width="70%"><progress value="85" max="100"></progress></td>
    <td>85%</td>
  </tr>
  <tr>
    <td><strong>C</strong></td>
    <td width="70%"><progress value="82" max="100"></progress></td>
    <td>82%</td>
  </tr>
  <tr>
    <td><strong>CSS3</strong></td>
    <td width="70%"><progress value="80" max="100"></progress></td>
    <td>80%</td>
  </tr>
  <tr>
    <td><strong>JavaScript</strong></td>
    <td width="70%"><progress value="75" max="100"></progress></td>
    <td>75%</td>
  </tr>
  <tr>
    <td><strong>SQL</strong></td>
    <td width="70%"><progress value="65" max="100"></progress></td>
    <td>65%</td>
  </tr>
  <tr>
    <td><strong>Java</strong></td>
    <td width="70%"><progress value="52" max="100"></progress></td>
    <td>52%</td>
  </tr>
  <tr>
    <td><strong>Flutter</strong></td>
    <td width="70%"><progress value="45" max="100"></progress></td>
    <td>45%</td>
  </tr>
  <tr>
    <td><strong>Express.js</strong></td>
    <td width="70%"><progress value="30" max="100"></progress></td>
    <td>30%</td>
  </tr>
  <tr>
    <td><strong>Python</strong></td>
    <td width="70%"><progress value="15" max="100"></progress></td>
    <td>Learning...</td>
  </tr>
</table>

## 📈 GitHub Stats

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=MdKhanBahadurSadi&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true&hide_border=true&bg_color=0d1117&cache_seconds=1800" alt="GitHub Stats" />
<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=MdKhanBahadurSadi&layout=compact&theme=tokyonight&hide_border=true&bg_color=0d1117&cache_seconds=1800" alt="Top Languages" />

</div>

<div align="center">

[![GitHub Streak](https://streak-stats.demolab.com?user=MdKhanBahadurSadi&theme=tokyonight&hide_border=true&background=0d1117&cache_seconds=1800)](https://git.io/streak-stats)

</div>

---

## 🚀 Projects Focus

<div align="center">

| 🔬 AI & ML Integration | 🌍 Real-World Solutions | 📱 Smart Applications |
|:---:|:---:|:---:|
| Integrating AI into everyday products | Systems that solve actual problems | Scalable, efficient, and modern apps |

</div>

---

## 🌱 Current Goals

- 🤖 Master **Machine Learning & AI**
- 💼 Build impactful software products
- 💰 Transition into **professional freelancing**
- 🌍 Establish a global tech brand — **ITHLAYNIT**

---

## 🧠 Philosophy & Values

- 📖 Follow **Qur'an & Sunnah** in all aspects of life
- 🎯 Strive for **discipline, consistency, and purpose**
- 💡 Seek knowledge as an act of worship
- 🤝 Build with **honesty and integrity**

---

## 🌍 Personal Insights

- 📍 Based in **Khulna, Bangladesh**
- ❤️ Passionate about learning new technologies
- ⚡ Focused on **meaningful, real-world impact**
- 🕌 Faith-driven developer — building for a greater purpose

---

## 📬 Contact Me

<div align="center">

[![Email](https://img.shields.io/badge/Email-khanbahadursadi@gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:khanbahadursadi@gmail.com)
[![Website](https://img.shields.io/badge/Website-Visit_Now-6C63FF?style=for-the-badge&logo=google-chrome&logoColor=white)](http://www.mdkhanbahadursadi.ami.bd/)

</div>

---

<div align="center">

![Footer](https://capsule-render.vercel.app/api?type=waving&color=0:24243e,50:302b63,100:0f0c29&height=120&section=footer)

</div>
